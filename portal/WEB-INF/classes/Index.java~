     ...
     throws ServletException, IOException 
   {
      ...
      if ("browse".equals (action))
      {
         ...
         if ("Title".equals (category) && "asc".equals (order))
            highlight = 2;
         if ("Title".equals (category) && "desc".equals (order))
            highlight = 3;
      ...
      out.println ("<html>"+
                   ...
                   "<div id=\"sidebarnav\">"+
                     ...
                     "<p class=\"menusep\">Browse</p>"+
                            "<a "+(highlight==2?"class=\"active\" ":"")+
                              "href=\"?action=browse&category=Title&order=asc\">Title (A-Z)</a>"+
                            "<a "+(highlight==3?"class=\"active\" ":"")+
                              "href=\"?action=browse&category=Title&order=desc\">Title (Z-A)</a>"+
                            "<a "+(highlight==4?"class=\"active\" ":"")+
                              "href=\"?action=browse&category=Date&order=desc\">Date (now->)</a>"+
                            "<a "+(highlight==5?"class=\"active\" ":"")+
                              "href=\"?action=browse&category=Date&order=asc\">Date (->now)</a>"+
	 ...
	 display_results.create_document(request.getParameter("category"),browseResults,records_requested.getTotalRecords(),records_requested.getOffset(),maxResults,records_requested.getLastDisplayed(),order,configPath);
      ...
      else if ("about".equals (request.getParameter ("action")) || "submit".equals (request.getParameter ("action")))
      {
         content = "<div id=\"contentarea\">"+
                       ...
                       "<p>This site is run by the <a href=\"http>://www.nrf.ac.za\">South African "+ 
                       "National Research Foundation (NRF)</a> in collaboration with the Committee "+
                       "of Higher Education Librarians of South Africa (CHELSA).</p>"+
                       "<p>Metadata for all theses and dissertations produced in South Africa "+
      ...
      else
      {
         ...
         for ( int i=0; i<sources.getLength(); i++ )
         {
            ...
            else if ("http://dk.cput.ac.za/oai/oai2.cgi/request".equals (url))
               url = "Cape Peninsula University of Technology";
         ...
         for ( int i=0; i<items.getLength(); i++ )
         {
            ...
            String guid = item.getElementsByTagName ("guid").item(0).getFirstChild().getNodeValue();
            recentEntries += "<li><a href=\"?action=view&identifier="+guid+"\"><span class=\"snippet_title\">"+title+"</span></a><br/>"+
      ...
      out.println ("</div><div id=\"footer\">"+
            "Portal and Archive managed by NRF; CSS courtesy of <a href=\"http://www.free-css-templates.com/\">Free CSS Templates</a>"+
